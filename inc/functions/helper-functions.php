...
function idea_push_shortcode_replacement($ideaId,$setting,$voterId){
    ...
    $find = '[ideapush-custom-field-';
    $last_position = 0;
    $matches = array();


    while (($last_position = strpos($setting, $find, $last_position))!== false) {
    ...
    foreach ($matches as $match) {
        ...
        $next_end_bracket = strpos( $setting , ']', $match);
            
        $meta_name = substr ( $setting ,$match + strlen($find), $next_end_bracket - ($match + strlen($find)) );
        
        $meta_name = 'ideapush-custom-field-'.$meta_name;

        $meta_value = get_post_meta($ideaId, $meta_name, true);

        $replacement_array['['.$meta_name.']'] = $meta_value;
        
    }
    
    //loop through array
    foreach($replacement_array as $key => $value){
        $setting = str_replace($key,$value,$setting);    
    }
